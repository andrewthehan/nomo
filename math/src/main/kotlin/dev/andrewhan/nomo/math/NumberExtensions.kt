...
fun <NumberType : Number> NumberType.isPositive() = this > 0
fun <NumberType : Number> NumberType.isNegative() = this < 0
fun <NumberType : Number> NumberType.isZero() = this == 0

operator fun <NumberType : Number> NumberType.plus(other: NumberType): NumberType {
  return when (this) {
    is Long -> (this.toLong() + other.toLong()).cast()
    is Int -> (this.toInt() + other.toInt()).cast()
    is Short -> (this.toShort() + other.toShort()).cast()
    is Byte -> (this.toByte() + other.toByte()).cast()
    is Double -> (this.toDouble() + other.toDouble()).cast()
    is Float -> (this.toFloat() + other.toFloat()).cast()
    else -> throw AssertionError("Unknown numeric type: ${this::class}")
  }
}
