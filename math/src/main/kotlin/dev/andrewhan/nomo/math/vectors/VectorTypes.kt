...
class Vector1fInternal(x: Float) : AbstractVector<Float>(x), Vector1f {
  override val vectorProducer: (elementProvider: (Int) -> Float) -> Vector1f = {
    Vector1fInternal(it(0))
  }
}

class Vector2fInternal(x: Float, y: Float) : AbstractVector<Float>(x, y), Vector2f {
  override val vectorProducer: (elementProvider: (Int) -> Float) -> Vector2f = {
    Vector2fInternal(it(0), it(1))
  }
}

class Vector3fInternal(x: Float, y: Float, z: Float) : AbstractVector<Float>(x, y, z), Vector3f {
  override val vectorProducer: (elementProvider: (Int) -> Float) -> Vector3f = {
    Vector3fInternal(it(0), it(1), it(2))
  }
}

class Vector1iInternal(x: Int) : AbstractVector<Int>(x), Vector1i {
  override val vectorProducer: (elementProvider: (Int) -> Int) -> Vector1i = {
    Vector1iInternal(it(0))
  }
}

class Vector2iInternal(x: Int, y: Int) : AbstractVector<Int>(x, y), Vector2i {
  override val vectorProducer: (elementProvider: (Int) -> Int) -> Vector2i = {
    Vector2iInternal(it(0), it(1))
  }
}

class Vector3iInternal(x: Int, y: Int, z: Int) : AbstractVector<Int>(x, y, z), Vector3i {
  override val vectorProducer: (elementProvider: (Int) -> Int) -> Vector3i = {
    Vector3iInternal(it(0), it(1), it(2))
  }
}
