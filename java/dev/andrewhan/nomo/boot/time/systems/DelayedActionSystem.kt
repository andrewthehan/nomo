...
class DelayedActionSystem @Inject constructor(private val engine: NomoEngine) :
  NomoSystem<UpdateEvent>() {
  @OptIn(ExperimentalTime::class)
  override suspend fun handle(event: UpdateEvent) {
    engine.getComponents<DelayedActionComponent>().forEach { it.update(event.elapsed, engine) }
  }
}
